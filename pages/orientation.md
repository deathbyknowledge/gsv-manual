...
The most important idea is that GSV is not only a chat window. Chat is one surface inside a larger computer. Files can outlive a conversation. Library pages can become searchable knowledge. Packages can add apps and commands. Devices can run work where the hardware or private network actually lives. Integrations can bring messages in from external places.

## What Lives Where

- Conversations live in [Agents & Assistants](agents-assistants/index.md) and are usually opened through Chat.
- Documents, imports, and durable references live in [Files & Knowledge](files-knowledge/index.md).
- Windows, app launching, previews, browser automation, native web surfaces, repository browsing, and package apps live in [Apps & Desktop](apps-desktop/index.md).
...
- Open Library if the task is about long-term knowledge.
- Open Repositories if the task is about source history, diffs, package source, or a wiki/manual repository.
- Open the GSV console if the task is about system state, devices, packages, integrations, access, or settings.
- Use Terminal when the task is command-driven or the app does not expose the needed operation.
...
- Put reusable facts in files or Library, not only in the current conversation.
